...
2. Use `mutate()` to create a new column `variety` (this overwrites the existing column) that keeps the original `variety` value for any row where `acres` is greater than 100,000, and assigns the value `"Other"` otherwise *then*
...
This works, but returns a quite ugly chart.  Once again, we can add the same formatting options as we added to the bar chart to make it more readable.  One new argument appears here: `limits = c(0, NA)` inside `scale_y_continuous()` starts the y-axis at zero, following the zero-baseline principle from @sec-graphing-principles, and the `NA` leaves the upper limit for ggplot to choose from the data.
...
3. Re-total the acres by variety and year, so `"Other"` is a single line rather than twenty-five separate ones.
