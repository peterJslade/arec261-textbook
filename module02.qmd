...
1. **You save things as named objects.** An "object" is just a piece of data you have given a name — it might be a single number, a list of numbers, a whole table of data, or a result you computed. You create one with the **assignment operator**, `<-`. The same operator saves *any* kind of data; here are three objects of increasing richness:
    ...
    yield_province <- read.csv("yields.csv")   # a data frame: a whole table
    ```

    Each line reads as "let the name on the left *be* the thing on the right." `yield_field` is one value, `yield_farm` is a vector of four values (the `c()` function *combines* values into a vector), and `yield_province` is an entire table read in from a file. All three are now objects living in R's memory — you will see them appear in Positron's *Environment* pane — and you can refer to each by name from then on. (We use base R's `read.csv` here; later we switch to the tidyverse's `read_csv`, which is a little smarter about column types.)
