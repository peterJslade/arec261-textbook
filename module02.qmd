...
1. **You save things as named objects.** An "object" is just a piece of data you have given a name — it might be a single number, a list of numbers, a whole table of data, or a result you computed. You create one with the **assignment operator**, `<-`:

    ```r
    yield <- 42
    ```

    This reads as "let `yield` be 42." R now keeps an object called `yield` in memory (you will see it appear in Positron's *Environment* pane), and you can refer to it by name from then on.
...
2. **You run functions on those objects.** A **function** takes one or more objects, does something with them, and hands back a result:
    ...
    ```r
    mean(yield)      # run the 'mean' function on the object 'yield'
    ```
