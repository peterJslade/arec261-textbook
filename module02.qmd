...
You can access columns with the `$` operator.  For example, to get the column `yield` in our data frame `fields` we would type `fields$yield`.  You will even note that when you type `fields$` Positron will provide a dropdown of all the columns that you cna choose from.  
