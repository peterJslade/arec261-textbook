...
- `=SUM(B5:B9)` adds the values in the range.
- `=AVERAGE(B5:B9)` returns their mean.
- `=COUNT(B5:B9)` returns how many of the cells hold a number.
...
bushels produced by each field. Total acres can then be calculated with `=SUM(B5:B9)`, while
total production is `=SUM(D5:D9)`. Column E shows the formula used in each cell of column D
...
`=AVERAGE(B5:B9)` gives 188 acres -- the mean field size -- and `=COUNT(B5:B9)` gives 5.
...
them with commas, and you can include individual numbers: `=SUM(B5:B9,40)` adds 40 acres of
summerfallow that are not in the table. A range can also run across a row rather than down a
column. But be careful -- `=SUM(B5:D5)` would add acres, yield, and total bushels together.
...
If you add three new fields below the table, `=SUM(B5:B9)` will continue to total only the
