...
**(a)** Compute the minimum and maximum of `Yield`, and the range (maximum minus minimum).

**(b)** In a comment: the minimum is 1.9 bu/ac. If that row turned out to be a data error and was removed, which would change more -- the range or the IQR?

**(c)** In a comment: which of the two better describes the spread of typical canola yields?
...
# (a) Smallest and largest yield
min(rm_canola$Yield)
max(rm_canola$Yield)

# (a) The range
max(rm_canola$Yield) - min(rm_canola$Yield)

## I find a minimum of 1.9, a maximum of 61.0, and a range of 59.1 bu/ac.
## (b) The range: it is built from the two most extreme values, so
## removing the 1.9 moves it directly. The IQR depends only on the 25th
## and 75th percentiles, and one row at the bottom barely touches those.
## (c) The IQR. The range describes two extreme RM-years; the IQR
## describes the middle half of them.
')
```
