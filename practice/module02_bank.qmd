...
- [`rm_canola_1990plus.csv`](data/rm_canola_1990plus.csv) -- average **canola** yield in every Saskatchewan Rural Municipality, 1990--2025, one row per RM-year. **Section 2** uses this file: one crop, one unit, so its summary statistics mean something.
- [`rm_yields_1990plus_long.csv`](data/rm_yields_1990plus_long.csv) -- the same data for all **eight crops**, one row per RM-year-crop. **Sections 3 and 4** use this file. You worked with the wide version in Module 1; this is the long form, with `Crop` as a variable. **Lentils are recorded in pounds per acre; every other crop is in bushels per acre** -- the `Unit` column says which, and several questions turn on it.
...
rm_canola <- read_csv("data/rm_canola_1990plus.csv")
...
*These questions use the canola-only file, [`rm_canola_1990plus.csv`](data/rm_canola_1990plus.csv), in your project's `data/` folder. Answer in the Section 2 block of your script.*
...
```r
rm_canola <- read_csv("C:\\Users\\jordan\\Desktop\\stats stuff\\rm_canola_1990plus.csv")
```
...
- (b) `rm_canola <- read_csv("data/rm_canola_1990plus.csv")`.
...
- (a) `rm_canola <- read_csv("module_2/data/rm_canola_1990plus.csv")`.
- (b) The starting point changed: relative paths begin at the folder you opened. With `module_2/` open, the path is `data/rm_canola_1990plus.csv`.
- (c) From `AREC_261`, R goes into `module_2`, then into `data`, and finds `rm_canola_1990plus.csv` -- each `/` steps into a subfolder.
</details>
...
Open `rm_canola_1990plus.csv` in Positron's data viewer (click the file in the Explorer pane).
...
*From here on, use the full eight-crop file: read [`rm_yields_1990plus_long.csv`](data/rm_yields_1990plus_long.csv) into an object called `rm_yields` at the start of your Section 3 block. These questions use one function at a time; save intermediate results to objects where a later part needs them.*
