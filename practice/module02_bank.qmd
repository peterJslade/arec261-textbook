...
- [`rm_canola_yields_1990_2025.csv`](data/rm_canola_yields_1990_2025.csv) -- average **canola** yield in every Saskatchewan Rural Municipality, 1990--2025, one row per RM-year. **Section 2** uses this file: one crop, one unit, so its summary statistics mean something.
- [`rm_yields_1990_2025.csv`](data/rm_yields_1990_2025.csv) -- the same data for all **eight crops**, one row per RM-year-crop. **Sections 3 and 4** use this file. You worked with the wide version in Module 1; this is the long form, with `Crop` as a variable. **Lentils are recorded in pounds per acre; every other crop is in bushels per acre** -- the `Unit` column says which, and several questions turn on it.
...
*These questions use the canola-only file, [`rm_canola_yields_1990_2025.csv`](data/rm_canola_yields_1990_2025.csv), in your project's `data/` folder. Answer in the Section 2 block of your script.*
...
*From here on, use the full eight-crop file: read [`rm_yields_1990_2025.csv`](data/rm_yields_1990_2025.csv) into an object called `rm_yields` at the start of your Section 3 block. Each question uses two or three of the functions in sequence -- save each intermediate result to an object and work step by step (pipelines wait for Section 4).*
