...
This bank holds **40 questions**, ten in each section. Section 1 gives you the data inside the question; Sections 2--4 use the **Saskatchewan RM Crop Yields (long)** dataset. On the real test you receive **four questions -- one from each section** -- and you submit a **zipped project folder** containing `data/` (the csv, exactly as downloaded) and `code/` with **one R script per section** (`1_basics.R`, `2_reading.R`, `3_manipulation.R`, `4_pipelines.R`). Each question is designed to take about **8--10 minutes**.
...
**Grading model.** **10%** for script presentation -- a header block, comments in the style of the module, and a script that runs from top to bottom in a fresh session. The remaining **90%** is split evenly across the four questions, and evenly across the parts within each question. Parts are marked on **follow-through**: if an early part is wrong but the later parts are correctly worked *from your own earlier answer*, the later parts still earn full marks. Where a part asks for a sentence, write it as a comment in your script.
...
*These questions are about the building blocks: objects, vectors, data frames, and functions. No data file is needed -- the numbers are in the question. Answer in `code/1_basics.R`, with sentence answers written as comments.*
...
**(c)** In a comment: what does the `[1]` at the start of a printed result mean?
...
- (c) The value shown is the first element of the result; it matters only when a result is long enough to wrap across lines.
</details>
...
*These questions use [`rm_yields_1990plus_long.csv`](data/rm_yields_1990plus_long.csv) in your project's `data/` folder. Answer in `code/2_reading.R`.*
...
*Same dataset. Answer in `code/3_manipulation.R`. These questions use one function at a time; save intermediate results to objects where a later part needs them.*
...
*Same dataset. Answer in `code/4_pipelines.R`. Write multi-step work as pipelines with `|>`, one step per line.*
