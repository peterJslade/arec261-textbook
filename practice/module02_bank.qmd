...
- (b) Yes -- everything sits between about 2 and 61 bu/ac, sensible for canola; the years run 1990--2025 as advertised; and `summary()` reports no NA counts, so no missing values.
...
- (a) Mean **28.29**, median **26.9**, sd **10.06**.
...
- (c) Oats, RM **369**, in **2025**, at **165.1 bu/ac**.
...
**(d)** In a comment: rewrite the condition "yield above 60 and the crop is *not* Spring Wheat" using `!`, and say what each of `&`, `|`, `!` requires.
...
- (d) `filter(rm_yields, Yield > 60 & !Crop == "Spring Wheat")`. `&` needs both conditions true, `|` needs at least one true, `!` flips a condition.
</details>

### Question 24

**(a)** Make a version of the data with only `Year`, `RM`, `Crop`, and `Yield` -- once by *naming what you keep*, once by *dropping* what you don't want -- and confirm the two give the same column names.
...
```r
names(select(rm_yields, Year, RM, Crop, Yield))
names(select(rm_yields, -Unit))
...
- (a) Identical: `Year`, `RM`, `Crop`, `Yield` both ways.
