...
**(d)** Compute the average canola yield for `Year = 1995` and for `Year = 2020` separately. Using just these two numbers, explain how a difference between early and recent years would add to the pooled spread from (b).
...
- (c) Pooling years mixes lower early values with higher recent ones, so total spread combines within-year variation *and* the difference between years.
- (d) 1995 canola averages well below 2020 (early years are markedly lower than recent ones in the data). When both are pooled, that early-vs-recent gap adds to the spread on top of the within-year variation.
- (e) Because recent years read higher than early ones in the data, a 36-year average sits below recent levels and understates a recent-year expectation.
</details>
...
**(e)** The Saskatchewan RM canola CV jumped to about 0.39 in the 2021 drought, while this Manitoba wheat CV stayed near 0.25. Using only these numbers, state which dataset's yields were disturbed *more* by the drought in relative terms.
...
- (e) The **Saskatchewan** data was disturbed more in relative terms — its drought-year CV (~0.39) is much higher than the Manitoba figure (~0.25), so its yields spread out more relative to their average.
</details>
...
**(d)** Compare durum's CV to canola's (Question 22). State which crop's provincial yields are steadier, citing the two CVs.
...
**(d)** Contrast this with `AAC BRANDON (BW 932)` from Question 42 in terms of how much data backs each variety's average.
...
**(b)** Try the same lookup for a variety-municipality-year you expect to be suppressed. Describe what your lookup returns.

**(c)** Explain why a lookup can succeed (return a number) for one variety-place-year but return a blank or error for another, in terms of the `Reported` status.

**(d)** Explain why you would want to check the `Reported` column *before* trusting a looked-up yield.

**(e)** State, in one sentence, why a blank lookup result here should not be read as "a low yield."

<details><summary>Answer</summary>

- (a) ≈ 65.1 bu/ac.
- (b) A suppressed combination returns a blank `Yield_bu_ac` (Reported = FALSE) — no number.
- (c) Reported rows carry a yield; suppressed rows have a blank yield, so the lookup finds a row but no value.
- (d) The `Reported` flag tells you whether the blank means "withheld" rather than a real measurement, so you know how to treat it.
- (e) A blank means the yield was withheld, not measured — reading it as "low" would invent data.
</details>
...
**(a)** Use `COUNTIF` to count the total reported rows, and `COUNTIF` to count the suppressed rows (from Question 41 you have these).
...
- (d) That crop may not be grown/reported in that province-year, so the yield cell is blank.
...
- (c) The sharpest dip is **2021** (down to ~22 bu/ac from ~41 in 2019).
...
**(c)** Compare the width of this histogram's spread to the canola histogram from Question 61. Which crop's yields are more spread out in 2023?
...
**(b)** Describe the overall direction and identify the lowest year.

**(c)** Compare the lowest year here to the lowest year on your canola line chart (Question 63). Are they the same year?

**(d)** Explain what it means when two different crops share their worst year on these charts.

**(e)** Explain why reading the *same* low year across multiple crop charts is stronger evidence of a broad event than a single crop's chart alone.

<details><summary>Answer</summary>

- (a) Line chart, titled/labelled.
- (b) Rising overall; lowest year **2021** (~35 bu/ac).
- (c) Yes — both canola and barley bottom out in 2021.
- (d) It means the poor year hit both crops, pointing to a shared cause rather than a crop-specific one.
- (e) One crop dipping could be crop-specific; multiple crops dipping in the same year points to a broad, shared event.
</details>
...
**(c)** Contrast this shape with the Saskatchewan canola histogram from Question 61.

**(d)** This data has genuine low-end outliers by the 1.5×IQR rule (about 39 of them across all years). Explain how a *box plot* of the same data would display them.
...
**(d)** You want to show a general audience that "these top varieties yield similarly." Would you choose the box plot or a bar chart of means? Justify it in terms of audience and message.

**(e)** Explain one way an unlabelled or poorly titled version of your chart could mislead a reader.

<details><summary>Answer</summary>

- (a) Box plot, six varieties (AAC Brandon, Starbuck, Wheatland, Hockley, Viewfield, Redberry), titled/labelled.
- (b) Spread and consistency (some varieties more variable) plus outliers — a bar of means hides all of that.
- (c) The variety with the narrowest box/shortest whiskers is most consistent (e.g. AAC Hockley, IQR ≈ 12.7).
...
**(c)** Compare this low year to the low year in the Saskatchewan canola line chart (Question 63). Are they the same year?
...
**(d)** Explain why the count of flagged outliers is smaller for a single year (2023) than for all years pooled (Question 71).
...
**(c)** Explain what this bar chart shows well, and what it hides compared with the box plot from Question 72.
...
**(c)** Compare this single-variety histogram's spread to the all-varieties histogram from Question 71. Which is tighter?
...
**(d)** Explain what this bar chart shows well and what a box plot across years (Question 81) would add.
...
- (d) A box plot per province (Question 81), which separates the provinces.
...
**(c)** Compare this seeded-acres bar chart to a yield bar chart (Question 83). Does the same province lead both?
...
**(c)** Compare it to the low year on your canola line chart (Question 82). Are they the same?
...
**(c)** Compare this province-specific low year to the national low year from Question 82.
...
**(a)** Build a PivotTable of average yield by crop for 2019 and, separately, for 2025 (Unit = bu/ac).

**(b)** For each crop, compute the change from 2019 to 2025 (a fifth column, or by reading both tables).
...
- (a) Two by-crop tables (2019 and 2025).
...
**(c)** Compare this all-years ranking to the 2023-only ranking from Question 91(d). Is the order the same?
...
**(c)** Compare the 2021 ranking to the 2023 ranking (Question 91d). Does the drought year change which crop is on top?
...
**(b)** Describe the overall direction of the line and identify the lowest year.
...
- (b) Rising overall; lowest year **2021**.
...
- (b) Highest average ≈ **SY MANNESS** (~72); most-reported ≈ **AAC BRANDON (BW 932)** (~519 reports, mean ~60).
- (c) A variety grown/reported on fewer sites can post a high average without being widely reported.
- (d) The widely-reported average (AAC Brandon, n≈519) is more trustworthy than a high average on ~108 reports.
- (e) Averages from 1–2 reports are noisy and can top the list by chance; a ≥30 threshold keeps only well-supported estimates.
</details>
...
**(c)** Compare this single-year ranking to the all-years ranking from Question 101.
...
**(a)** Build a PivotTable of average yield by variety and turn it into a bar PivotChart (top varieties), titled with labelled axes.
...
- (a) Bar PivotChart of variety averages, titled/labelled.
- (b) Tallest ≈ SY MANNESS; shortest among top varieties ≈ AAC Redberry.
...
**(c)** Compare this ranking to the yield-per-acre ranking from Question 111. Does the same province lead both?
...
**(b)** Rank the provinces and compare the order to the 2023-only ranking from Question 111.
