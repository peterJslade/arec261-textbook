...
Enter the value **0.0842** into five separate cells.

**(a)** Format them, one each, as: General, Number with 2 decimals, Percentage with 1 decimal, Currency, and Scientific.

**(b)** Beside each, add a formula multiplying that cell by 1000.

**(c)** All five results in (b) are identical. Explain what this tells you about the difference between what a cell holds and what it shows.

<details><summary>Answer</summary>

- (a) `0.0842` · `0.08` · `8.4%` · `$0.08` · `8.42E-02`.
- (b) Every one returns **84.20**.
- (c) A number format changes only the **display**. The stored value is unchanged, and formulas always use the stored value. The percentage cell still holds 0.0842, not 8.4.
...
**(a)** Add a **Value ($)** column and a total.
...
- (b) **No.** Each row has its own price, so the reference must move with the row. Anchoring it with `$B$5` would price every bin as canola and overstate the total by about $30,000. Anchoring is not automatically the safe choice — it is right only when the formula must keep pointing at one cell.
