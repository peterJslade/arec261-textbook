...
> **Vocabulary note.** Everything here uses only Module 1 tools: mean/median/mode, range, IQR, variance, standard deviation, coefficient of variation, percentiles/quartiles, symmetric vs skewed shape, outliers (the 1.5×IQR rule), histograms, relative and absolute references, number and date formatting, and PivotTables.
...
*Build this sheet yourself. No dataset file is needed.*

Put the canola price **$14.20** in a single cell of its own, labelled. Then enter these five fields:
...
**(c)** Change the price cell to $12.00. All five revenues should update. If they do not, explain what is wrong with your formula.
...
- (c) All five update because every formula points at the same cell. If only the first row is right, the price reference was relative.
...
Enter the three prices **across a row**: Canola $14.20, Wheat $8.35, Barley $5.60. Underneath, enter five fields with a yield for each crop:
...
Type in this delivery log exactly as shown:
...
**(b)** The first gap should read 4. If yours shows something like `04-Jan-1900`, explain what happened and fix it.
...
- (b) Excel inherited the **date** format from the cells being subtracted. The answer is a plain number of days — change the cell to Number format.
- (c) `=A9-A5` = **25** days.
...
**(b)** Now write `=B4*B5-B6*B7` and record what it returns. Explain in one sentence why it is wrong.
...
**(a)** Format them, one each, as: General, Number with 2 decimals, Percentage with 1 decimal, Currency, and Scientific. Record what each displays.
...
Set up two labelled input cells: canola price **$14.20**/bu and **44.09** bushels per tonne. Then enter this log:
...
**(d)** Change the price to $11.50 and confirm every value updates. Explain which references had to be absolute and why.
...
- (d) Both input cells must be absolute. Fill either column down without the `$` signs and the reference walks down into blank cells, returning zero for every row after the first — with no error message.
