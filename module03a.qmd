...
Much like Ecel, `read_csv()` guesses each column's type from the first thousand values.  In the console above, we can see that `ticket_id` kept its zeros and `lot_code` stayed text. However, it still read `permit_book` as a number, prints it as `4.52e15`, and has already lost its last digits. We can force types in the `read_csv()` command by using the col_types argument.  This argument is a named list (EXPLAIN A BIT MORE)
...
We can also read Excel files directly in R using the package **readxl**. It installs with the tidyverse but is not loaded by `library(tidyverse)`, so load it separately.  Excel workbooks can have many sheets -- but defauly `readxl` will reade the first one, but we can direct it to read a different sheet using the sheet argument, which takes either the name of the sheet or an integer equal to the position of the sheet in the workbook (e.g., 2 for second sheet).
...
## Read in the first sheet of sask_barley_20205
...
## Read in the sheet "Barley_20205" from sask_barley_20205
...
## Read in the sheet "Barley_20205" from sask_barley_2025, skipping the first three rows
barley_no_skip <- read_excel(
  "practice/data/sask_barley_2025.xlsx",
  sheet = "Barley_2025"
)
...
Spreadsheets built by people often have something above the table: a title, a note about units, a blank row. R takes the first row as column names regardless, and what comes back looks like this:
