...
Spreadsheets built by people often have something above the table: a title, a note about units, a blank row. R takes the first row as column names regardless.  We can tell R to skip a certain number of lines using the `skip` argument.

The code below shows how to read in the barley data.  The first  attempts to read in the data return nonesense because the first reads the first sheet of the worksheet, not the data data table, the seonc ddoe't skip the first three rows. The third row gives us the data that we want.
...
An alternative to skip is to specify the range of cells that we want to import.  This can be handy for ignoring both rows and columns.
```{r}
#| eval: true
#| echo: false
console('
