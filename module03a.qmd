...
Note that in a csv file a comma delimits fields, in other files there might be a different **delimiter**.  To read these files you can use `read_delim()` and specifuy the delimiter with the delim argument.

Much like Ecel, `read_csv()` guesses each column's type from the first thousand values and prints what it decided. For the tickets file:
