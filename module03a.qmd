...
Building API requests by hand is fiddly, and this course does not ask you to. For the providers we use, someone has written an R package that builds the request and hands back a data frame -- the packages in @sec-data-packages are wrappers around an API.
...
For many APIs a developer has created a package that builds the request for you and returns the data directly as a data frame.

The **openmeteo** package is one example.  When you load this package you can use the function `weather_history()` to directly download data.  As with all functions, if you want to understand the arguments that you need to supply to weather history type `?weather_history` in your console.  In this case you will see that you need to supply at least three arguments, start, end and either hourly or daily:
...
After installation, one function call returns a data frame with the Saskatoon precipitation data for May 2024:
