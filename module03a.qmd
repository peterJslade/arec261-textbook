...
The file at a URL can be revised or replaced; in this case, the RM data are updated at least once a year. After reading the data, save a dated copy so that you retain the version used in your analysis.
...
The URL above records where the data came from, and the date in the filename records when this copy was obtained. If the online file changes later, this dated snapshot preserves the data used for the analysis. Strictly speaking, `write_csv()` saves the values after R has read them rather than preserving the exact original file, but it captures the important practice at this stage: keep the version you actually used.
...
The body is JSON. JSON can contain groups inside other groups, so `resp_body_json()` represents it as a nested R list (see the discussion of lists in @sec-r-basics). The outer `weather_json` list contains an element named `daily`, which is itself a list. Inside `daily`, the elements named `time` and `precipitation_sum` are equal-length vectors. We can extract those vectors with `$` and use them as columns in a data frame:
...
# Extract the vectors and put them into a data frame
...
Each `$` moves down one level of the list: `weather_json$daily` selects the daily list, and `$time` or `$precipitation_sum` selects a vector inside it. `data.frame()` then places those vectors in columns. Every API's JSON is shaped differently, so the first time you use one, run `str(weather_json)` and read the provider's documentation rather than guessing element names. Save the raw response too, with `writeLines(resp_body_string(weather_response), "data/raw/saskatoon_2024-05.json")`, for the same reason as the raw CSV.

### Packages that wrap an API {#sec-data-packages}

Getting data from an API (as above) is a bit of work.  We need to get the JSON data and then parse it ourselves.  For many APIs a developer has created a package that allows you to call the API and obtain the data directly in a data frame.  

As mentioned above, the **openmeteo** package is one example.  When you load this package you can use the function `weather_history()` to directly download data.  As with all functions, if you want to understand the arguments that you need to supply to weather history type `?weather_history` in your console.  In this case you will see that you need to supply at least three arguments, start, end and either hourly or daily:

- start, end: Required. Start and end dates in ISO 8601 (e.g. "2020-12-31").

- hourly, daily: At least one required. A weather variable accepted by the API, or list thereof. Details provided in the help description.

Additionally, you can supply the units you want the data in (e.g., "mm" or "cm") and the timezone the hours/days refer to.

Once again we can install the package using the following code:
...
After installation, one function call returns a data frame with the Saskatoon precipitaiton data for May 2024:
