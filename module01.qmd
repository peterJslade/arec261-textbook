...
- **Centre.** Where is the *central point* of the data? (Mean, median, mode.)
...
Now let's actually do some data analysis. The most basic question you can ask about a set of numbers is: "where is the centre of the data?" — the single value that best locates the middle of everything. There are three common answers, and which one is "right" depends on what you mean by "the middle." Note that the centre is *not* the same as a "typical" value: as we will see, the mean can land in a spot where no actual observation sits, so treat these measures as ways to *locate* the data, not necessarily to describe a representative case.
...
You now have two answers to "where is the centre of the data?" — and comparing them is more useful than either one alone. The reason goes back to **outliers**: a few extreme values pull the *mean* toward them but barely move the *median*. So the **gap between the mean and the median tells you whether extreme values are at work, and in which direction.**
