...
The workbook below shows this happening. It works through the same conversion four
times: once with a relative reference (which is fine), once pointing at the factor cell
*without* locking it — where you can watch the reference drift down and the answers quietly
fall apart — and once with `$` signs holding it in place. The last block is a grid where one
formula is copied both across and down, which is the situation that needs the mixed forms
`B$5` and `$B5`.
