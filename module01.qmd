...
In this chapter, we will learn a) the statistics and charts that can be used to summarize the centre, position, spread, and shape of the data; b) how to interpret these statistics and charts; and c) how to calculate them in Excel.  Along the way we will also learn a lot about how Excel operates.
...
### Three Real Workbooks {.unnumbered}

Advice is easier to follow once you have seen it done. Below are three workbooks published by real organizations. Open each one and look at the *structure* — the sheet tabs along the bottom, where the documentation lives, how the data is laid out — rather than the numbers themselves.

::: {.callout-tip collapse="true"}
## Example 1 — Saskatchewan Crop Planning Guide (a large, well-organized workbook)

The Ministry of Agriculture's **Crop Planning Guide** estimates the cost of production for every major crop in the province. It is a genuinely complex file — 35 sheets — and it is worth studying because it does almost everything this section recommends:

- **A *Disclaimer* sheet first**, stating what the file is, who made it, and when.
- **A *General Assumptions* sheet** listing 22 numbered assumptions behind every number in the workbook — crop prices, seeding rates, how machinery depreciation is calculated. This is transparency in practice: a reader can check the reasoning without reverse-engineering formulas.
- **One sheet per crop**, all built on an *identical template* (revenue at the top, then variable expenses, then other expenses, then returns). Learn to read one sheet and you can read all thirty.
- **A *Summary Calculator*** that pulls from the individual crop sheets with cross-sheet references such as `=MBarley!$B$9`, and adds a "My Farm" column where a producer enters their own acres. Analysis is separated from data.

Notice the trade-off, too: with 35 sheets, naming discipline slips (`MBarley`, `Ypeas`, `Fbean`, `Chickpeas, large`), and many cells carry unrounded values such as `46.625288340000004`. Even good workbooks are rarely perfect.
...
[Download this workbook](textbook_examples/sask_crop_planning_guide_2026.xlsx)
:::

::: {.callout-tip collapse="true"}
## Example 2 — Manitoba primary elevator capacity (a published data series)

Manitoba Agriculture publishes a historical series on **primary elevators and grain storage capacity**, running from 1963 to the present. It is a much simpler file — two sheets — and it shows what a *published data table* typically looks like:

- The source and a link to the publishing branch sit in the **top rows**, before the data.
- The table has a **two-row header**: the variable names on one row (`Year`, `Number`, `Total Capacity`) and the *units* on the next (`000 tonnes`, `000 bushels`).

That header is convenient for a human and awkward for software. Excel and R both expect a single header row, so a file like this needs tidying before analysis — exactly the kind of cleaning we discussed above, and a good argument for keeping a **raw** sheet and a separate **cleaned** sheet.
...
[Download this workbook](textbook_examples/mb_primary_elevator_capacity_2025.xlsx)
:::

::: {.callout-tip collapse="true"}
## Example 3 — Saskatchewan RM yields, 2024–2025 (a clean analysis-ready table)

This is an extract from Saskatchewan's **rural municipality (RM) crop yield** data, filtered to 2024 and 2025. It is the simplest of the three and the closest to what you want your *own* data sheets to look like:

- **One header row**, with plain variable names.
- **One row per observation** — here, one RM in one year.
- **One column per variable** — `Year`, `RM`, then a column for each crop's yield.
- No titles, notes, or blank rows above the data; the table starts in cell `A1`.

Blank cells appear where a crop was not grown in that RM, which is honest — a missing value is not the same as a zero. Data in this shape can be summarized, sorted, filtered, and charted immediately, with no cleaning first.
...
[Download this workbook](textbook_examples/sask_rm_yields_2024_2025.xlsx)
:::
...
A cell also has a particular number-format.  Examples are *General*, *Number*, *Date*, and *Currency*.  Excel will usually default to formatting a cell as *General* and will sometimes autodetect when you enter a date and change the format to date.  You can alter the format of a cell by right-clicking on it and choosing **Format Cells…** (@fig-format-menu), which opens a dialog where you pick a category and set its options — for a plain number, for example, how many decimal places to show and whether to use a thousands separator (@fig-format-dialog).  The most common reasons you would want to ensure that a cell is formatted properly is when you are using a Date or when you want to ensure numbers are rounded.

![Right-click a cell and choose **Format Cells…** to change how its value is displayed.](images/excel_format_cells_menu.png){#fig-format-menu width=85%}

![The **Format Cells** dialog. Here cell A1 holds the value `100`; the *Number* category with two decimal places makes it *display* as `100.00`. The underlying value is unchanged — only its appearance is.](images/excel_format_cells_dialog.png){#fig-format-dialog width=85%}
...
Here is a live example workbook — Saskatchewan barley varieties in 2025, with each variety's total acres and its acreage-weighted average yield across the province. Have a poke around before we go on.

::: {.excel-embed}
<iframe width="402" height="346" frameborder="0" scrolling="no" src="https://usaskca1-my.sharepoint.com/personal/pjs998_usask_ca/_layouts/15/Doc.aspx?sourcedoc={b1a180eb-5099-42f2-9a76-a24c61c44bf5}&action=embedview&wdAllowInteractivity=False&wdHideGridlines=True&wdHideHeaders=True&wdDownloadButton=True&wdInConfigurator=True&wdInConfigurator=True"></iframe>
:::

[Download the workbook](practice/data/sask_barley_2025.xlsx) · [or the CSV](practice/data/sask_barley_2025.csv)
