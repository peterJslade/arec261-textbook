...
That third bullet — a date "internally stored as a number" — points at an idea worth pausing on: **the value a cell holds and the way it looks on screen are two different things.** This is what *cell formatting* controls. Formatting changes only the appearance; the underlying value is untouched. Type `0.45` and format it as a **percent** and the cell *displays* `45%`, but Excel still stores and calculates with `0.45`. Format `1000` as **currency** and it *shows* `$1,000.00`, while any formula referencing it still sees the plain number `1000` — so `=A1*2` gives `2000`. The number is wearing a costume, not becoming a different number. (And this is exactly why a date behaves like a number: `2026-09-15` is really a serial count of days, which is what lets you subtract two dates to get the days between them.)

The practical catch is **displayed rounding**. Put `2.4` in two cells and format both to show *zero* decimals — each displays `2`. But `=A1+A2` adds the true values and returns `4.8`, which then displays as `5`. The cells appear to claim "2 + 2 = 5." Nothing is broken; you are only seeing rounded versions of honest numbers. So when a total looks off by a rounding error, suspect the display before you go hunting for a bug. The number-format controls live on the **Home** tab (the *Number* group): General / Number / Currency / Percent / Date, plus buttons for decimal places and commas. Font, borders, and cell colour live nearby and are purely cosmetic — they make a sheet readable but never change a calculation. **Format for the human reading the sheet; the computer always works with the underlying value.**
