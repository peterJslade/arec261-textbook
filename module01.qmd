...
2. Calculate measures of central tendency (mean, median, mode) and spread (range, variance, standard deviation, coefficient of variation) for a dataset, and use the mean-vs-median comparison to judge whether a distribution is symmetric or skewed.
3. Calculate percentiles and quartiles, and interpret what they mean.
4. Use conditional functions (`IF`, `SUMIF`, `COUNTIF`, `AVERAGEIF`, and dynamic criteria like `">"&AVERAGE(...)`) to do calculations on subsets of data, and handle blank cells correctly.
5. Use lookup functions (`VLOOKUP`, `XLOOKUP`, `INDEX`/`MATCH`), including two-key lookups, to combine information from multiple tables.
6. Distinguish wide from long data, and build PivotTables and PivotCharts to summarize data.
7. Create and interpret basic charts — bar, line, histogram, and box-and-whisker — with clear titles and axis labels, and understand why pie charts are usually a poor choice.
...
### The Shape of a Distribution: Skew {.unnumbered}
...
1. Download the [`rm_yields_1990plus.csv`](practice/data/rm_yields_1990plus.csv) dataset, filter to a single recent year and one crop, and produce a one-page summary of the distribution of yields using the techniques in this module (mean/median, spread, a histogram). Remember to isolate the year *before* computing statistics.
2. Using the same dataset, compare the mean and median canola yield in 2021 (a drought year) to a normal year. Why do the two measures differ, and what does the gap tell you about the shape of the distribution?
