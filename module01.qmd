...
The gap between what a cell *shows* and what it *holds* is behind four mistakes worth recognizing. The workbook below demonstrates each one — every row shows the result alongside the formula that produced it:

- **Display rounding.** Two cells holding `2.4` displayed as `2` add up to `5`. The sheet appears to say 2 + 2 = 5.
- **A date stored as text.** It looks identical to a real date, but arithmetic on it returns `#VALUE!`.
- **A number stored as text.** The dangerous one: `SUM` *silently ignores it*, so your total is wrong with no error at all.
- **Leading zeros stripped.** A farm ID of `01234` becomes `1234`, so it no longer matches the same ID elsewhere.
...
Rather than list that in a table, the workbook below *shows* it. Each row computes a result in one column and prints the formula that produced it in the next — so you can see the syntax and the answer side by side. Work down through the operators, the PEMDAS examples, and the step-by-step walk through `=3+6*(5+4)/3-7`. Pay particular attention to the last two rows: `=100/2*5` and `=100/(2*5)` use the same three numbers and give wildly different answers.

::: {.workbook-callout}
::: {.excel-embed}
<iframe width="402" height="346" frameborder="0" scrolling="no" src="https://usaskca1-my.sharepoint.com/personal/pjs998_usask_ca/_layouts/15/Doc.aspx?sourcedoc=%7Bc02b36ae-e4f1-4cb9-aa41-da843db59da7%7D&action=embedview&wdAllowInteractivity=False&wdDownloadButton=True&wdInConfigurator=True"></iframe>
:::
...
If you are ever unsure of the order, add parentheses — they never hurt.
