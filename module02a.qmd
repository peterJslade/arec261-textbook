...
A vector contains values of one type, and a data frame is a rectangular collection of equal-length vectors. A **list** is more flexible: each element can contain a different kind or amount of information. One element might be a number, another a vector and another a list of its own. A list inside another list is called a **nested list**.
...
Lists often appear when data do not fit naturally into one rectangular table. In @sec-api-r, for example, R converts a JSON response from a weather API into a nested list. We then extract equal-length vectors from that list and use them as columns in a data frame.
