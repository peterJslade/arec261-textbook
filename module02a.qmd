...
This chapter covers why we are using R, how to install it, and the mechanics of the
console, scripts, and R's basic building blocks.
...
**R** is a programming language designed for statistical computing. It addresses each of these limitations. Your analysis is a **script** -- a plain-text file that can be re-run, shared, version-controlled, and composed with other scripts. R can handle datasets of tens of millions of rows, and there are thousands of packages written by statisticians around the world.

Learning **R** means learning to program.  Instead of being limited to what an app was built to do, you write the steps yourself.  AI can now write a lot of that code for you.  That makes programming skills more useful rather than less: someone still has to know what to ask for and whether the answer is right.  That argument is in @sec-ai.
...
This is the basic pattern in R: create objects with `<-`, and run functions on them.  Sometimes we will save the results as new objects when we want to keep them, and we may perform operations on these objects.
...
You should only ever type code directly into the console (as we did above) for quick, throwaway calculations.  For any real analysis -- anything you want to re-run, check, share, or hand in -- you need your code written down in a file you can save.

That file is a **script**: a plain text file (ending in `.R`) where you *write and save* your code. The script is the permanent recipe for your analysis: you edit it, save it, and can re-run the whole thing later or share it with someone else.
...
![After running the script, the **console** shows the results and the **Variables** pane lists the objects that were created.](images/positron_script_after_run.png){#fig-script-after}
...
- `x <- c(1, 2, 3, 4, 5)` creates a **vector** of the numbers 1 through 5 and stores it under the name `x`.
- `mean(x)` calls the `mean` function on `x` and prints the result.

The object `x` is now stored in **R**, and it stays there until we exit this session or overwrite `x` with a new value.

This will be the last screenshot of **Positron** that I will show in the textbook.  From now on I will just show the code, and sometimes the results of this code.    
...
If a vector is numeric then we can do arithmetic operations on it.  For example, say we have a vector of yields in bu per acre and we want to convert it to tonnes per hectare, we can simply multiply the entire vector by a constant conversion factor.
...
But you will almost always **read** a data frame from a file, which is the subject of the next chapter.

You can access columns with the `$` operator.  For example, to get the column `yield` in our data frame `fields` we would type `fields$yield`.  When you type `fields$`, Positron provides a dropdown of the columns you can choose from.
...
R largely operates by performing *functions* on *objects*.  We have already seen that there is a function called `mean()`.  There is a nearly infinite variety of functions.

**R** functions generally require arguments.  For example, in `mean(fields$yield)`, the function is `mean()` and the function's argument is `fields$yield`.
...
There are two ways to supply the arguments.  First, write them in the correct order without naming them.  Second, name the arguments, in which case the order does not matter.
...
2. **Use comments to explain *why*, not *what*.** Anyone can read the code and see what it does. The comment should tell them why you did it that way.
3. **Use descriptive object names.** `yields`, not `x1`. `mean_yield`, not `m`.
4. **Break long calculations into named steps.** Assign intermediate results to objects with meaningful names.
5. **Make your script re-runnable from scratch.** If you have to run commands in a specific order outside the script, something is wrong.
...
# Date: 2026-09-22
# Description:
#   Computes total production and the acreage-weighted average
#   yield for the five wheat fields from Module 1.
# ---
...
# 3. Farm average (weighted by acres)
total_bu / sum(acres)
```

Six months from now you can read this and know what it does.
