...
The Variables pane now lists `a` and `b`, but nothing from the `a*5` line -- displaying a result and saving one are different things.
...
R stores things as **objects**. An object might contain a single number, a sequence of numbers, a table of data, a statistical model, or even a graph. For now, we need two kinds of objects: **vectors**, which hold a sequence of values, and **data frames**, which hold data in rows and columns.
...
A vector holds one variable. Real datasets usually contain many, and in R a table of data is called a **data frame**. The shape is familiar from Excel: each row is an observation, and each column is a variable.
...
Each row is a field; `field_id`, `region`, and `yield` are variables; and each column is itself a vector. We built this data frame by hand so you can see how it is constructed, but you will almost always **read** a data frame from a file, which is the subject of the next chapter.
...
A data frame is a table, and its columns are vectors. The next chapters cover more convenient tools for choosing rows and columns, creating new variables, and summarizing data frames.

### Functions {.unnumbered}

Most of our work in R consists of applying **functions** to objects. A function is an instruction that takes some input, does something with it, and returns a result. We have already used several: `c()`, `mean()`, `print()`, `data.frame()`.
...
There are two ways to supply the arguments.  First, write them in the correct order without naming them.  Second, name the arguments, in which case the order does not matter.  Named arguments are particularly useful when a function has several options, because the code then tells the reader what each value means.
...
2. **Use comments to explain decisions, not to restate the code.** A line beginning with `#` is a **comment**: R ignores it, so it is a note for the person reading the code. `mean_yield <- mean(yields)  # calculate mean yield` adds nothing; the code already says that. A comment like `# Weight by acres because fields differ substantially in size` tells the reader something the code cannot.
