...
Check again before trusting the result. The ranges are now sane and the Brandon spellings have collapsed into one. The cleaned file goes to `output/` and the raw file is never edited. If a corrected version of the raw data arrives, rerunning the script applies the same rules and rebuilds the cleaned file.
