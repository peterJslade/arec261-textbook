...
## Issue 9 and 10: Moisture has text and NA values recorded as 9999.  We can first change the type to numeric using parse_number() and then change any values above 100 to NA using the if_else() function.  Save the resulting data frame as records_moisture_fix and check the moisture_percent column.

records_moisture_fix <- records_variety_fix |>
