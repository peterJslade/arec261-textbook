...
The yield minimum is a missing-value code and the maximum is twenty times a real yield, and together they pull the mean eighteen bushels above the median. The N rate minimum and maximum are in different units. The next two columns refuse to summarize at all: the unit suffix and the missing-value codes make them text, which is issue 2 doing its damage before any analysis starts. The last row shows what happens if the text is ignored and moisture is summarized over the numbers that remain, which is what Excel's `AVERAGE` quietly does: the 9999 missing-value code is a number, so it sails into the calculation and drags the mean to 299.3% moisture. The median, sitting in the middle of the ordered values, barely notices.
...
#| echo: false
console('
## Install janitor package (if not installed already) -- I have commented it out because its already isntalled
...
## Issue 2: text in harvest weight. We can strip the unit text from the weights and make them numeric using the parse_number() function.  This function extracts the numeric part of a string and converts it to a numeric value.  Save the resulting data frame as records_weight_fix and check the harvest_weight column.

records_weight_fix <- records_new_names |>
  mutate(harvest_weight = parse_number(harvest_weight))

summary(records_weight_fix$harvest_weight)

## Issue 3: Duplicate rows.  We can remove duplicate rows using the distinct() function.  It removes any rows that are identical to another row in the dataset.  Save the resulting data frame as records_unique and check the number of rows.

records_unique <- distinct(records_weight_fix)

nrow(records_unique)



## Issue 4 and 5: Impossible and NA yields. We can change yields below 0 and above 200 to NA using the if_else() function. This deals with yields that are reported as -99 (a typical missing value code) and others that are negative.  Save the resulting data frame as records_yield_fix and check the yield_bu_ac column.

records_yield_fix <- records_unique |>
...
## Issue 6: Nitrogen units vary. We can convert nitrogen rates below 1 to kg/ha by multiplying them by 1000 using the if_else() function.  Save the resulting data frame as records_n_rate_fix and check the n_rate_kg_ha column.

records_n_rate_fix <- records_yield_fix |>
    mutate(n_rate_kg_ha = if_else(n_rate_kg_ha < 1,
                                  n_rate_kg_ha * 1000,
                                  n_rate_kg_ha))

summary(records_n_rate_fix$n_rate_kg_ha)


## Issue 7: Inconsistent variety names. We can fix the variety names in the same way using the if_else() function.  The three arguments would say if variety is one of "Brandon", "Brandon AAC", or "Brndon", then replace it with "AAC Brandon", otherwise leave it as is.  Save the resulting data frame as records_variety_fix and count the varieties in the variety_name column.

records_variety_fix <- records_n_rate_fix |>
