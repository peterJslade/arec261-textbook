...
1. [`rm_yields_2015_2024.csv`](practice/data/rm_yields_2015_2024.csv) -- one row per RM per year. It contains 2,950 rows for ten years and six crops. Yields are measured in bushels per acre.
2. [`station_precip.csv`](practice/data/station_precip.csv) -- one row per weather station per year. It contains May to August precipitation for sixteen stations and a count of days with no reading.
3. [`rm_lookup.csv`](practice/data/rm_lookup.csv) -- one row per RM. It contains the RM name, census division, 2021 population and the closest of the sixteen weather stations.
...
[Download the finished merge workbook](textbook_examples/mod03_merge_excel.xlsx) -- the three queries with their Applied Steps intact.
...
::: {.video-callout}
::: {.callout-note collapse="true" title="Video walkthrough of this section"}
{{< video https://youtu.be/cO26aLUeSoM >}}
:::
:::
...
Finally, we merge the `with_station` dataset with the precipitation data using another left join.
