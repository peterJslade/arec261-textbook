...
Perhaps the simplest way to visualize the shape of the distribution is through a histogram.  A histogram divides the range of the data into **bins** and plots a bar showing how many observations fall in each bin. It shows you at a glance whether the data is roughly **symmetric** (balanced around the centre), **skewed** (a long tail on one side, the mean-above-median pattern from @sec-central-tendency), or has **outliers** (a few values sitting far from the rest).

**How to make one in Excel**: select the data, **Insert → Statistical Chart → Histogram**. Right-click the horizontal axis → **Format Axis** to control the bin width.
...
::: {.workbook-callout}
The **Histogram** tab holds the barley yields in a single column, ready to chart.
