...
Now that you can get numbers into Excel and compute with them, we turn to what those numbers *mean*: how to summarize a column of yields so a human can grasp it, and how to read what the summary is telling you. Everything here uses the Excel skills from the previous two chapters.
...
This comes out to 52.0 bu/ac, a little *below* the plain 52.5. The gap is telling you that the bigger fields yielded slightly below the smaller ones, so counting every field equally overstated the farm's real average.
...
So a handy rule of thumb: compute both, and look at the gap. A big gap is a flag that outliers are present and that you should report the *median* (and go look at what those extreme values are). A small gap means the two agree and the mean is fine.

(When the extremes cluster on one side like this, the data is **skewed** -- mean above median is called *right-skewed*, mean below median *left-skewed*. Histograms in @sec-charts show this shape directly.)
...
Percentiles are how crop insurance programs define "bad years" (e.g., "a yield in the lowest 10th percentile"), how government agencies define poverty thresholds, and how agronomists describe the performance of a variety ("top-quartile yield").
