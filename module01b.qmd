...
`IF` is the function that makes a decision. It looks at a condition and returns one of two values depending on whether that condition is true. The `IF` function has three arguments that are seperated by commas: (1) the condition to test, (2) the value to return if the condition is true, and (3) the value to return if the condition is false. Both the condition and the values returned can reference other cells.
...
Column E of the workbook below uses `IF` to label each field `"Yes"` or `"No"`. Note the quotation marks: text inside a formula always needs them.
...
```
=IF(D5>50, IF(B5="Canola", "Yes", "No"), "No")
```
...
The argument order is different from `AVERAGEIF`, which catches people out. The range being
...
The workbook below has the fields to sort and filter yourself.

::: {.workbook-callout}
<!-- iframe embed goes here once the workbook is uploaded to SharePoint; follow the
mod01_sum.xlsx block in module01a.qmd -->

[Download this workbook](textbook_examples/mod01_sorting_filtering.xlsx)  
The workbook includes a practice sheet to fill in yourself.
:::

<!-- Video walkthrough callout goes here once the sorting-and-filtering video is up:
::: {.callout-note collapse="true" title="Video walkthrough of this workbook"}
{{< video URL >}}
:::
-->
...
tells you how much data each average rests on. An average of four observations and an average
of four hundred look identical until you show the count.
