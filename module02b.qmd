...
# Every example in this chapter runs against this data frame. Students load it
# themselves with read_csv("field_yields.csv"); the path differs here because
# the book renders from the project root.
...
When you write just `"canola_yields_2025.csv"` -- a filename with no folder path -- R looks for that file in one specific place called the **working directory**: the folder R currently considers "here." If the file is not in that folder, you get:
...
that are powerful enough to express most of what you will ever need.  Most data manipulation is
a *sequence of simple operations*, and with the right small vocabulary the complex
...
Throughout this chapter, when an example just shows a pipeline without `<-`, assume we are
...
#| echo: false
console('
yields |> select(field_id, region, yield_bu_acre)
')
```
...
#| echo: false
console('
yields |> mutate(yield_t_ha = yield_bu_acre * 0.0560)
')
```
...
#| echo: false
console('
yields |> arrange(desc(yield_bu_acre))
')
```

`arrange(yield_bu_acre)` sorts the other way, and you can give it more than one column --
`arrange(region, desc(yield_bu_acre))` sorts by region, then by yield within each region.
...
you cannot explain, ask what it does before you run it.  Module 5 covers this properly —
