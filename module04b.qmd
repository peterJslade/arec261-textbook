...
The workbooks embedded through the chapter already contain the data, so you only need these files to rebuild the charts from scratch.
...
A pie chart shows the shares of a whole, and it works best there are a small number of slices (@sec-graphing-principles covers when to be wary of them). Suppose we want to show how 2025 barley acreage splits across varieties. We can easily create a pie chart by selecting data the same way as in FIg11.1 and choosing a par chart instead of a bar chart.  The result below is horrendous.

![The first attempt: one slice per variety.](images/mod04b_pie_start.png){#fig-pie-start .screenshot}

To fix this we can group all varieties except the four largest into an `Other` category. Five slices is something a reader can actually take in.  If we want to show more varieties, a pie chart is definitely not the bast option.

After that we need to do the same formatting work as the bar chart: a descriptive title, and data labels through **Add chart element > Data Labels**. One particularly helpful formatting idea is directly labelling each slice usinh **Data Labels > Inside End** as shown in @fig-pie-labels.
