...
The examples in this chapter use Saskatchewan Crop Insurance Corporation variety records [@scic_smp]: the acreage of barley varieties in 2025 for the bar and pie charts, and acreage by year from [`sask_variety_yields.csv`](practice/data/sask_variety_yields.csv) for the line charts. The R chapter (@sec-graphing-r) builds the same charts in code.
...
1. Create a chart from selected worksheet data, and restrict it by filtering.
2. Change a chart's type, title, axis labels and bounds.
3. Check and correct the categories and series a chart uses with **Select Data**.
4. Group small categories into an "Other" slice for a pie chart.
5. Plot several series on one line chart, directly and with a PivotChart.
6. Format a chart without adding unnecessary decoration.
:::
...
<!-- TODO: export the all-variety acres chart (All variety bar chart sheet) from Excel as a
large PNG (Save as Picture > PNG) and place it here as images/mod04b_bar_first.png.
...
<!-- TODO: export the finished acres chart (Formated bar chart sheet) from Excel as a large
PNG (Save as Picture > PNG) and place it here as images/mod04b_bar_final.png.
...
<!-- TODO: screenshot or PNG export of the finished pie. Save it to the Desktop and it goes
here as images/mod04b_pie_final.png.
